        ...
        function(ffprobe2, cb) {
          ...
          self2._getFfmpegPath(function(err, ffmpeg2) {
            if (err) {
              ...
              var ffprobe22 = path$1.join(path$1.dirname(ffmpeg2), name2);
...
const require$$0 = {
  ...
  description,
  scripts,
  "ffmpeg-static": { "binary-path-env-var": "FFMPEG_BIN", "binary-release-tag-env-var": "FFMPEG_BINARY_RELEASE", "binary-release-tag": "b6.1.1", "binaries-url-env-var": "FFMPEG_BINARIES_URL", "executable-base-name": "ffmpeg" },
  repository,
  keywords,
  authors,
  contributors,
...
const ffmpegInstaller = /* @__PURE__ */ getDefaultExportFromCjs(ffmpegStaticExports);
if (ffmpegInstaller) {
...
function createWindow() {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    win.loadFile(path__namespace.join(__dirname, "../index.html"));
  }
}
...
electron.ipcMain.handle("start-conversion", async (event, { filePath, targetFolder }) => {
  const fileName = path__namespace.basename(filePath, path__namespace.extname(filePath));
  const outputPath = path__namespace.join(targetFolder, `${fileName}_landscape.mp4`);
